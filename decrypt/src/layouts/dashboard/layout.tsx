...
import { checkModuleAccess } from 'src/auth/utils/permission-utils';
...
export function DashboardLayout({
  ...
  const canDisplayItemByModule = (navItem: NavItemDataProps): boolean => {
    // Check if user has access to the specific module
    if (navItem.module) {
      const hasModuleAccess = checkModuleAccess(companies, selectedCompanyId, { module: navItem.module });
      if (!hasModuleAccess) {
        return false;
      }
    }
    ...
    if (navItem.modules && navItem.modules.length > 0) {
      const hasAnyModuleAccess = checkModuleAccess(companies, selectedCompanyId, { modules: navItem.modules });
  ...
  const checkNavPermissions = (navItem: NavItemDataProps): boolean => {
    // First check role-based permissions
    if (navItem.allowedRoles && !canDisplayItemByRole(navItem.allowedRoles)) {
      return false;
    }

    // Then check module-based access
    return canDisplayItemByModule(navItem);
  };
    ...
    const headerSlots: HeaderSectionProps['slots'] = {
      ...
      bottomArea: isNavHorizontal ? (
        <NavHorizontal
          ...
          checkPermissions={checkNavPermissions}
        />
      ) : null,
      leftArea: (
        <>
