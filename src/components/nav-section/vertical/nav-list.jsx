...
export function NavList({ data, depth, render, slotProps, checkPermissions, enabledRootRedirect }) {
  ...
  const isChildActive = data.children?.some((child) =>
    isActiveLink(pathname, child.path, child.deepMatch)
  );

  const shouldStayOpen = isActive || isChildActive;

  const { value: open, onFalse: onClose, onToggle } = useBoolean(shouldStayOpen);
