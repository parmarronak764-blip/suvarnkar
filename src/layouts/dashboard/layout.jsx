...
export function DashboardLayout({ sx, cssVars, children, slotProps, layoutQuery = 'lg' }) {
  ...
  const checkPermissions = (item) => {
    ...
    return true; // If no permission specified, show the item
  };

  const companyArr = [];

  if (accountInfo?.company_users?.length > 0) {
    accountInfo.company_users.forEach((account) => {
      companyArr.push({ ...account.company });
    });
  }
