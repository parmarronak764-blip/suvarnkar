...
export const useWastages = () => {
  ...
  // Get endpoints
  const ENDPOINTS = API_ROUTES.WASTAGES;
  ...
  // Fetch list of wastages with pagination
  ...
  // Create new wastage (supports both single and bulk creation)
  const createItem = useCallback(
    async (itemData) => {
      try {
        ...
        if (itemData.isBulkEntry && itemData.wastages && Array.isArray(itemData.wastages)) {
          // Handle bulk creation using bulk endpoint
          const bulkRequestBody = {
            company_id: companyId,
            wastages: itemData.wastages.map((wastage) => ({
              wastage_code: wastage.wastage_code,
          ...
          if (response.success) {
            ...
            return {
              ...
              bulkDetails: {
                ...
                createdWastages: response.created_wastages || [],
                existingWastages: response.existing_wastages || [],
              },
            };
          }
          return { success: false, message: 'Failed to create wastages' };
        } else {
          // Handle single creation
          const requestBody = {
            wastage_code: itemData.wastage_code,
            charge_type: itemData.charge_type,
            wastage_percentage: parseFloat(itemData.wastage_percentage),
            company_id: companyId,
          };
        ...
        if (err.message && err.message.includes('duplicate key value violates unique constraint')) {
          return {
            success: false,
            message:
              'A wastage with this code already exists for your company. Please use a different code.',
          };
        }

        return { success: false, message: err.message || 'Failed to create wastage' };
      }
    },
    [callApi, ENDPOINTS, getCompanyId]
  );
  ...
  // Delete wastage (soft delete by setting is_active to false)
  ...
  const deleteItems = useCallback(
    async (ids) => {
      ...
      try {
        ...
        return { success: false, message: 'Failed to delete wastages' };
      } catch (err) {
        return { success: false, message: err.message };
      }
    },
