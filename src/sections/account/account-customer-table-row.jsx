...
export function AccountCustomerTableRow({ row, selected, onSelectRow, onEditRow }) {
  ...
  const contactNumber =
    row.whatsapp_country_code && row.whatsapp_number
      ? `${row.whatsapp_country_code} ${row.whatsapp_number}`
      : row.other_phone_country_code && row.other_phone_number
        ? `${row.other_phone_country_code} ${row.other_phone_number}`
        : '-';
  ...
  return (
    <>
      <TableRow hover selected={selected} aria-checked={selected} tabIndex={-1}>
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{row.customer_group_name || '-'}</TableCell>
