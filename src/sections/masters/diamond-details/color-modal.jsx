...
export function ColorModal({ open, onClose, currentItem, onSave, loading = false }) {
    ...
    try {
      if (data.isBulkEntry) {
        ...
        if (validItems.length === 0) {
          toast.error('Please enter at least one color name');
          return;
        }
  ...
  const title = isEdit ? 'Edit Diamond Color' : 'Add Diamond Color';
          ...
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            {isBulkEntry ? (
              ...
              <Field.Text
                name="name"
                label="Color Name"
                placeholder="Enter diamond color (e.g., D, E, F, G)"
                required
              />
            )}
          </Stack>
